...
class BlackWhiteNestedIndexBased:
    ...
    def print_meta_data_tofile(self):
        ...
        f_meta.write(str(self.parsed.xmax) + " " +
                     str(self.parsed.ymax) + "\n")
        f_meta.write("#depth\n")
        ...
        for action in self.parsed.black_action_list:
            f_meta.write(action.action_name +
        ...
        for i in range(self.parsed.depth):
            f_meta.write(self.make_string(self.move_variables[i][0]) + " " + self.make_string(
                self.move_variables[i][1]) + " " + self.make_string(self.move_variables[i][2]) + "\n")
            # appending vars:
            if (i % 2 == 0):
        ...
        f_meta.write(self.make_string(
            self.forall_position_variables[0]) + " " + self.make_string(self.forall_position_variables[1]) + "\n")
        # state vars:
        f_meta.write("#statevars\n")
        for i in range(self.parsed.depth+1):
            f_meta.write(" ".join(str(x)
                         for x in self.predicate_variables[i]) + "\n")
        ...
        self.output_unsat_index_string += ",".join(str(x)
                                                   for x in unsat_cert_vars)
    ...
    def generate_counter_index_constraint(self, variables, constraint_pair):
        self.encoding.append(
            ['# computing counter bounds for constraints,' + str(constraint_pair) + ' :'])
        assert ("?c" in constraint_pair[0])
        lsc.add_circuit(self.gates_generator, variables,
                        int(constraint_pair[1]))
        return self.gates_generator.output_gate
    ...
    def generate_position_equalities_with_adder_and_subtractors(self, x_variables, y_variables, constraint):
        ...
        result_y_variables = []
        self.encoding.append(
            ['# computing x variables for constraints,' + str(constraint) + ' add/sub/none:'])
        ...
        if ('+' in constraint[0]):
            ...
            result_x_variables = addc.adder_circuit(
                self.gates_generator, x_variables, num_to_add)
        elif ('-' in constraint[0]):
            split_x_constraint = constraint[0].split("-")
            assert ("?x" == split_x_constraint[0])
            num_to_sub = int(split_x_constraint[1])
            self.encoding.append(['# subtractor circuit'])
            result_x_variables = addc.subtractor_circuit(
                self.gates_generator, x_variables, num_to_sub)
        ...
        self.encoding.append(
            ['# computing y variables for constraints,' + str(constraint) + ' add/sub/none:'])
        ...
        if ('+' in constraint[1]):
            ...
            self.encoding.append(['# adder circuit'])
            result_y_variables = addc.adder_circuit(
                self.gates_generator, y_variables, num_to_add)
        elif ('-' in constraint[1]):
            split_y_constraint = constraint[1].split("-")
            assert ("?y" == split_y_constraint[0])
            num_to_sub = int(split_y_constraint[1])
            self.encoding.append(['# subtractor circuit'])
            result_y_variables = addc.subtractor_circuit(
                self.gates_generator, y_variables, num_to_sub)
        ...
        if ("?x" in constraint[0] or "?c" in constraint[0]):
            ...
            self.gates_generator.complete_equality_gate(
                result_x_variables, self.forall_position_variables[0])
            ...
            x_constraint_binary_format = self.generate_binary_format(
                self.forall_position_variables[0], x_value)
        ...
        if ("?y" in constraint[1] or "?c" in constraint[1]):
            ...
            self.gates_generator.complete_equality_gate(
                result_y_variables, self.forall_position_variables[1])
            ...
            y_constraint_binary_format = self.generate_binary_format(
                self.forall_position_variables[1], y_value)
    ...
    def generate_counter_forall_equality_gate(self, time_step):
        ...
        if (self.parsed.x_flag == 1):
            self.gates_generator.complete_equality_gate(
                self.move_variables[time_step][1], self.forall_position_variables[0])
        ...
        if (self.parsed.y_flag == 1):
            self.gates_generator.complete_equality_gate(
                self.move_variables[time_step][2], self.forall_position_variables[1])
            counter_equality_gates.append(self.gates_generator.output_gate)
        else:
    ...
    def generate_quantifier_blocks(self):
        ...
        self.quantifier_block.append(['# ' + str(self.num_black_action_variables) + '/' + str(self.num_white_action_variables) + ' (black/white) Action variables, ' + str(
            self.num_x_index_variables) + ' and ' + str(self.num_y_index_variables) + ' (action parameter) index variables (x, y), and game stop variables : '])
        for i in range(self.parsed.depth):
            ...
            if (i % 2 == 0):
                ...
                self.quantifier_block.append(
                    ['exists(' +str((self.win_variables[i][0])) + ')'])
            else:
                ...
                if (len(exists_action_vars) != 0):
                    self.quantifier_block.append(
                        ['exists(' + ', '.join(str(x) for x in exists_action_vars) + ')'])
                assert (len(cur_all_action_vars) != 0)
                self.quantifier_block.append(
                    ['forall(' + ', '.join(str(x) for x in cur_all_action_vars) + ')'])
                # if illegal move is present, then it is existential:
                if (len(self.move_variables[i][3]) != 0):
                    self.quantifier_block.append(
                        ['# white illegal variable: '])
                    self.quantifier_block.append(
                        ['exists(' + str(self.move_variables[i][3][0]) + ')'])
                # add the ww variable to intermediate encoding
        ...
        if (self.num_black_goal_constraints > 1):
            ...
            self.quantifier_block.append(
                ['exists(' + ', '.join(str(x) for x in self.disjunction_goal_boolean_variables) + ')'])
        ...
        if (len(all_black_goal_vars) > 0):
            self.quantifier_block.append(
                ['exists(' + ', '.join(str(x) for x in all_black_goal_vars) + ')'])
        ...
        if (self.num_white_goal_constraints > 1):
            all_forall_white_goal_vars.extend(
                self.forall_conjunction_white_goal_boolean_variables)
        if (len(all_forall_white_goal_vars) > 0):
            self.quantifier_block.append(
                ['forall(' + ', '.join(str(x) for x in all_forall_white_goal_vars) + ')'])
        ...
        if (self.extra_white_quantifier_alternation == 1):
            self.quantifier_block.append(
                ['exists(' + ', '.join(str(x) for x in self.single_white_goal_disjunct_boolean_variables) + ')'])
        ...
        if (len(exists_index_variables) != 0):
            self.quantifier_block.append(
                ['# exists symbolic index variables: '])
            self.quantifier_block.append(
                ['exists(' + ', '.join(str(x) for x in exists_index_variables) + ')'])
        assert (len(forall_index_variables) != 0)
        self.quantifier_block.append(['# Forall index variables: '])
        self.quantifier_block.append(
            ['forall(' + ', '.join(str(x) for x in forall_index_variables) + ')'])
        ...
        if (self.parsed.counter_flag == 1):
            ...
            for i in range(self.parsed.depth+1):
                self.quantifier_block.append(
                    ['exists(' + ', '.join(str(x) for x in self.counter_variables[i]) + ')'])

        # Finally predicate variables for each time step:
        self.quantifier_block.append(['# Predicate variables: '])
        for i in range(self.parsed.depth+1):
            self.quantifier_block.append(
                ['exists(' + ', '.join(str(x) for x in self.predicate_variables[i]) + ')'])
    ...
    def generate_black_transition(self, time_step):
        ...
        self.encoding.append(
            ['# Player 1 (black) transition function for time step ' + str(time_step) + ': '])
        ...
        if (self.upperlimit_black_actions != self.num_black_actions):
            ...
            lsc.add_circuit(
                self.gates_generator, self.move_variables[time_step][0], self.num_black_actions)
        ...
        for i in range(self.num_black_actions):
            ...
            binary_format_clause = self.generate_binary_format(
                self.move_variables[time_step][0], i)
            ...
            for index_constraint in self.parsed.black_action_list[i].positive_indexbounds:
                ...
                bound_result = self.generate_index_constraint(
                    self.move_variables[time_step][1], self.move_variables[time_step][2], index_constraint)
                # print(bound_result)
            ...
            for precondition in self.parsed.black_action_list[i].positive_preconditions:
                ...
                if ("lt" in predicate and "?c" in constraint_pair):
                    self.encoding.append(
                        ['# counter bound constraints with ' + str(constraint_pair[1]) + ":"])
                    bound_result = self.generate_counter_index_constraint(
                        self.counter_variables[time_step], constraint_pair)

                    # only when the x parameters are true then we imply the bound result:
                    bound_branch_condition_gate = self.generate_counter_forall_equality_gate(
                        time_step)
                    self.gates_generator.if_then_gate(
                        bound_branch_condition_gate, bound_result)
                    ...
                    temp_then_constraint_output_gates.append(self.generate_if_then_predicate_constraint(
                        cur_equality_output_gate, predicate, time_step, "pos"))
            ...
            for effect in self.parsed.black_action_list[i].positive_effects:
                ...
                if ("inc" == predicate):
                    ...
                    self.encoding.append(
                        ['# adder circuit for counter increment:'])
                    increased_counter_variables = addc.adder_circuit(
                        self.gates_generator, self.counter_variables[time_step], int(constraint_pair[1]))
                    # complete equality with next time step counter variables, i.e., increasing counter:
                    self.gates_generator.complete_equality_gate(
                        self.counter_variables[time_step+1], increased_counter_variables)
                    ...
                    self.gates_generator.if_then_gate(
                        counter_forall_equality_gate, counter_increased_equality_gate)
                    temp_then_constraint_output_gates.append(
                        self.gates_generator.output_gate)
                    ...
                    self.gates_generator.complete_equality_gate(
                        self.counter_variables[time_step], self.counter_variables[time_step+1])
                    self.gates_generator.if_then_gate(
                ...
                #         int(self.win_variables[time_step][0]))
                    ...
                    if ("?c" == constraint_pair[0]):
                        cur_equality_output_gate = self.generate_position_equalities_with_adder_and_subtractors(
                            self.counter_variables[time_step], self.move_variables[time_step][2], constraint_pair)
                    elif ("?c" == constraint_pair[1]):
                        cur_equality_output_gate = self.generate_position_equalities_with_adder_and_subtractors(
                            self.move_variables[time_step][1], self.counter_variables[time_step], constraint_pair)
                    else:
                        cur_equality_output_gate = self.generate_position_equalities_with_adder_and_subtractors(
                            self.move_variables[time_step][1], self.move_variables[time_step][2], constraint_pair)
                    touched_position_output_gates.append(
                        cur_equality_output_gate)
                    # time step + 1 because these are effects:
                    temp_then_constraint_output_gates.append(self.generate_if_then_predicate_constraint(
                        cur_equality_output_gate, predicate, time_step + 1, "pos"))
            # constraints for postive effects:
            for effect in self.parsed.black_action_list[i].negative_effects:
                ...
                # time step + 1 because these are effects:
                temp_then_constraint_output_gates.append(self.generate_if_then_predicate_constraint(
                    cur_equality_output_gate, predicate, time_step + 1, "neg"))
            ...
            self.gates_generator.or_gate(
                [self.gates_generator.output_gate, propagation_output_gate])
            temp_then_constraint_output_gates.append(
                self.gates_generator.output_gate)
        ...
        self.transition_step_output_gates.append(
            self.gates_generator.output_gate)
    ...
    def generate_white_transition(self, time_step):
        ...
        self.encoding.append(
            ['# Player 2 (white) transition function for time step ' + str(time_step) + ': '])
        ...
        assert (len(self.move_variables[time_step][4])
                == self.parsed.max_white_preconditions)
        cur_precondition_boolean_variables = self.move_variables[time_step][4]
        print(cur_precondition_boolean_variables)
        ...
        if (self.upperlimit_white_actions != self.num_white_actions):
            ...
            lsc.add_circuit(
                self.gates_generator, self.move_variables[time_step][0], self.num_white_actions)
            self.white_lessthan_circuit_output_gate = self.gates_generator.output_gate
            bound_variable_output_gates.append(
                self.white_lessthan_circuit_output_gate)
        ...
        self.gates_generator.single_equality_gate(
            cur_bound_boolean_var, final_bound_output_gate)
        # self.transition_step_output_gates.append(self.gates_generator.output_gate)
        current_transition_step_output_gates.append(
            self.gates_generator.output_gate)
        ...
        for i in range(self.num_white_actions):
            ...
            assert (len(cur_precondition_boolean_variables)
                    == self.parsed.max_white_preconditions)
            temp_precondition_boolean_variables = list(
                cur_precondition_boolean_variables)
            ...
            for precondition in self.parsed.white_action_list[i].positive_preconditions:
                ...
                if ("lt" in predicate and "?c" in constraint_pair):
                    ...
                    temp_cur_precondition_bool = temp_precondition_boolean_variables.pop(
                        0)
                    # single equality gate between precondition var and bound:
                    self.gates_generator.single_equality_gate(
                        bound_result, temp_cur_precondition_bool)
                    bool_equality_gate_counter = self.gates_generator.output_gate

                    # computing equality output gates:
                    cur_equality_output_gate = self.generate_counter_forall_equality_gate(
                        time_step)
                    # if then constraints, in the specific branch:
                    self.gates_generator.or_gate(
                        [-cur_action_binary_output_gate, -cur_equality_output_gate, bool_equality_gate_counter])
                    # self.transition_step_output_gates.append(self.gates_generator.output_gate)
                    current_transition_step_output_gates.append(
                        self.gates_generator.output_gate)
                else:
                    cur_equality_output_gate = self.generate_position_equalities_with_adder_and_subtractors(
                        self.move_variables[time_step][1], self.move_variables[time_step][2], constraint_pair)
                    cur_precondition_pos_then_gate = self.generate_then_predicate_constraint(
                        predicate, time_step, "pos")
                    # if the branch is equality branch, then boolean variables must be same as then gate:
                    # generate if then constraint with the precondition boolean variable:
                    temp_cur_precondition_bool = temp_precondition_boolean_variables.pop(
                        0)
                    ...
                    self.gates_generator.single_equality_gate(
                        cur_precondition_pos_then_gate, temp_cur_precondition_bool)
                    # if then constraints, in the specific branch:
                    self.gates_generator.or_gate(
                        [-cur_action_binary_output_gate, -cur_equality_output_gate, self.gates_generator.output_gate])
                    # self.transition_step_output_gates.append(self.gates_generator.output_gate)
                    current_transition_step_output_gates.append(
                        self.gates_generator.output_gate)
            # constraints for negative precondition:
            for precondition in self.parsed.white_action_list[i].negative_preconditions:
                ...
                cur_equality_output_gate = self.generate_position_equalities_with_adder_and_subtractors(
                    self.move_variables[time_step][1], self.move_variables[time_step][2], constraint_pair)
                cur_precondition_neg_then_gate = self.generate_then_predicate_constraint(
                    predicate, time_step, "neg")
                # if the branch is equality branch, then boolean variables must be same as then gate:
                # generate if then constraint with the precondition boolean variable:
                temp_cur_precondition_bool = temp_precondition_boolean_variables.pop(
                    0)
                # single equality gate between the bool and the output gate:
                self.gates_generator.single_equality_gate(
                    cur_precondition_neg_then_gate, temp_cur_precondition_bool)
                # if then constraints, in the specific branch:
                self.gates_generator.or_gate(
                    [-cur_action_binary_output_gate, -cur_equality_output_gate, self.gates_generator.output_gate])
                # self.transition_step_output_gates.append(self.gates_generator.output_gate)
        ...
        for i in range(self.num_white_actions):
            ...
            for effect in self.parsed.white_action_list[i].positive_effects:
                ...
                if ("inc" == predicate):
                    ...
                    # complete equality with next time step counter variables, i.e., increasing counter:
                    self.encoding.append(
                        ['# equality gate for next counter vars and increased counter:'])
                    self.gates_generator.complete_equality_gate(
                        self.counter_variables[time_step+1], increased_counter_variables)
                    counter_increased_equality_gate = self.gates_generator.output_gate
                    # implying the counter increased gate with counter forall equality gate:
                    self.encoding.append(
                        ['# if then gate for increased equality:'])
                    self.gates_generator.if_then_gate(
                        counter_forall_equality_gate, counter_increased_equality_gate)
                    then_constraint_output_gates.append(
                        self.gates_generator.output_gate)
                    ...
                    # first equality gate for counter variables:
                    self.encoding.append(
                        ['# counter propagation equality gate:'])
                    self.gates_generator.complete_equality_gate(
                        self.counter_variables[time_step], self.counter_variables[time_step+1])
                    self.encoding.append(
                        ['# in all other branches we propagate counter:'])
                    self.gates_generator.if_then_gate(
                        -counter_forall_equality_gate, self.gates_generator.output_gate)
                    then_constraint_output_gates.append(
                        self.gates_generator.output_gate)
                    ...
                    touched_position_output_gates.append(
                        cur_equality_output_gate)
                    # time step + 1 because these are effects:
                    then_constraint_output_gates.append(self.generate_if_then_predicate_constraint(
                        cur_equality_output_gate, predicate, time_step + 1, "pos"))
            # constraints for postive effects:
            ...
            self.gates_generator.and_gate(
                [valid_move_output_gate, cur_action_binary_output_gate])
            if_condition_output_gate = self.gates_generator.output_gate

            # if then constraint:
            self.gates_generator.if_then_gate(
                if_condition_output_gate, then_constraint_final_output_gate)
            # self.transition_step_output_gates.append(self.gates_generator.output_gate)
            current_transition_step_output_gates.append(
                self.gates_generator.output_gate)
    ...
    def generate_first_dummy_transition(self, time_step):
        ...
        self.encoding.append(
            ['# Dummy black move at time step ' + str(time_step) + ': '])
    ...
    def generate_initial_gate(self):
        ...
        for position in self.parsed.black_initial_positions:
            ...
            binary_format_clause1 = self.generate_binary_format(
                self.forall_position_variables[0], position[0])
            self.gates_generator.and_gate(binary_format_clause1)
            binary_output_gate1 = self.gates_generator.output_gate
            binary_format_clause2 = self.generate_binary_format(
                self.forall_position_variables[1], position[1])
            self.gates_generator.and_gate(binary_format_clause2)
            binary_output_gate2 = self.gates_generator.output_gate
            self.gates_generator.and_gate(
                [binary_output_gate1, binary_output_gate2])
            black_position_output_gates.append(
                self.gates_generator.output_gate)
        ...
        for position in self.parsed.white_initial_positions:
            ...
            self.gates_generator.and_gate(
                [binary_output_gate1, binary_output_gate2])
            white_position_output_gates.append(
                self.gates_generator.output_gate)
        ...
        if (len(black_position_output_gates) != 0 or len(white_position_output_gates) != 0):
            ...
            if (len(cur_condition_gates) != 0):
                ...
                initial_step_output_gates.append(
                    self.gates_generator.output_gate)
        ...
        if (self.parsed.counter_flag == 0):
            ...
            if (len(self.parsed.counter_initials) == 0):
                ...
                for c_initial in self.parsed.counter_initials:
                    ...
                    binary_format_forall_x = self.generate_binary_format(
                        self.forall_position_variables[0], c_initial[0])
                    ...
                    binary_format_counter_value = self.generate_binary_format(
                        self.counter_variables[0], c_initial[2])
                    ...
                    self.gates_generator.if_then_gate(
                        forall_x_output_gate, counter_value)
                    step_counter_implication_gates.append(
                        self.gates_generator.output_gate)
            ...
            if (len(initial_step_output_gates) != 0):
                ...
                # conjunction with counter zero gate:
                self.gates_generator.and_gate(
                    [final_counter_output_gate, -self.predicate_variables[0][0]])
                # we set every position to unoccupied:
                self.initial_output_gate = self.gates_generator.output_gate
    ...
    def generate_black_goal_gate(self):
        ...
        for single_conjunction_index in range(self.num_black_goal_constraints):
            ...
            if (self.num_black_goal_constraints > 1):
                ...
                temp_binary_variables = self.generate_binary_format(
                    self.disjunction_goal_boolean_variables, single_conjunction_index)
            ...
            for constraint in single_conjunction:
                if ("False" in constraint):
                    ...
                    self.gates_generator.or_gate([])
                    single_constraint_output_gates.append(
                        self.gates_generator.output_gate)
                elif ("nlt" in constraint):
                    assert (" " not in constraint)
                    # passing x variables and y variables along with index constraint:
                    bound_result = self.generate_index_constraint(
                        self.black_goal_index_variables[0], self.black_goal_index_variables[1], constraint)
                    ...
                    cur_equality_gate = self.generate_position_equalities_with_adder_and_subtractors(
                        self.black_goal_index_variables[0], self.black_goal_index_variables[1], constraint_pair)
                    # for now no negations in goal condition:
                    if (negated == True):
                        cur_constraint_gate = self.generate_if_then_predicate_constraint(
                            cur_equality_gate, predicate, self.parsed.depth, "neg")
                        # print("negated")
                    else:
                        cur_constraint_gate = self.generate_if_then_predicate_constraint(
                            cur_equality_gate, predicate, self.parsed.depth, "pos")
        ...
        if (self.num_black_goal_constraints > 1):
            if (self.disjunction_goal_upper_limit != self.num_black_goal_constraints):
                lsc.add_circuit(
                    self.gates_generator, self.disjunction_goal_boolean_variables, self.num_black_goal_constraints)
                black_goal_step_output_gates.append(
                    self.gates_generator.output_gate)
        ...
        # Final goal gate:
        self.encoding.append(
            ['# And gate for black goal constraints, at index ' + str(self.parsed.depth)])
    ...
    def generate_white_goal_gate(self):
        ...
        if (self.makermaker_game == 1):
            ...
            if (self.num_white_goal_constraints == 1 and self.white_single_goal_num_constraints == 1):
                split_condition = self.parsed.white_goal_constraints[0][0].strip(
                    ")").split("(")
                predicate = split_condition[0]
                ...
                cur_equality_gate = self.generate_position_equalities_with_adder_and_subtractors(
                    self.forall_position_variables[0], self.forall_position_variables[1], constraint_pair)
                ...
                for white_goal_conjunction_index in range(self.num_white_goal_constraints):
                    ...
                    self.encoding.append(
                        ['# computing white goal forall index: '])
                    # if there is only one constraint, then we do not need the universal variables:
                    if (self.num_white_goal_constraints > 1):
                        temp_binary_variables = self.generate_binary_format(
                            self.forall_conjunction_white_goal_boolean_variables, white_goal_conjunction_index)
                        self.gates_generator.and_gate(temp_binary_variables)
                    ...
                    for single_white_goal_disjunction_index in range(self.white_single_goal_num_constraints):
                        self.encoding.append(
                            ['# computing single white goal disjunct index: '])
                        temp_binary_variables = self.generate_binary_format(
                            self.single_white_goal_disjunct_boolean_variables, single_white_goal_disjunction_index)
                        self.gates_generator.and_gate(temp_binary_variables)
                        ...
                        cur_constraint = self.parsed.white_goal_constraints[
                            white_goal_conjunction_index][single_white_goal_disjunction_index]
                        # ==========================================================================================================================
                        ...
                        if ("nlt" in cur_constraint):
                            ...
                            if (self.num_white_goal_constraints > 1):
                                cur_if_bound_gates.append(
                                    white_goal_conjunction_index_binary_gate)

                            # passing x variables and y variables along with index constraint:
                            bound_result = self.generate_index_constraint(
                                self.white_goal_index_variables[0], self.white_goal_index_variables[1], cur_constraint)
                            ...
                            if (self.num_white_goal_constraints > 1):
                                ...
                                self.gates_generator.if_then_gate(
                                    cur_if_bound_final_gate, -single_white_goal_disjunction_index_binary_gate)
                                if self.gates_generator.output_gate not in white_goal_step_output_gates:
                                    white_goal_step_output_gates.append(
                                        self.gates_generator.output_gate)
                        ...
                        # assuming there are no lt or nlt in the constraint:
                            ...
                            split_condition = cur_constraint.strip(
                                ")").split("(")
                            predicate = split_condition[0]
                            # asserting the predicate is always white, for now:
                            # we allow other predicates, but not negation for now:
                            # assert(predicate == "white")
                            self.encoding.append(['# computing white goal constraint in white_goal[' + str(
                                white_goal_conjunction_index) + "][" + str(single_white_goal_disjunction_index) + ']'])
                            constraint_pair = split_condition[1].split(",")
                            # we need to handle any adders and subtractors if both variables are present:
                            cur_equality_gate = self.generate_position_equalities_with_adder_and_subtractors(
                                self.white_goal_index_variables[0], self.white_goal_index_variables[1], constraint_pair)
                            ...
                            if (self.num_white_goal_constraints > 1):
                                self.gates_generator.and_gate(
                                    [white_goal_conjunction_index_binary_gate, single_white_goal_disjunction_index_binary_gate, cur_equality_gate])
                                cur_final_if_condition_output_gate = self.gates_generator.output_gate
                            else:
                                self.gates_generator.and_gate(
                                    [single_white_goal_disjunction_index_binary_gate, cur_equality_gate])
                                cur_final_if_condition_output_gate = self.gates_generator.output_gate
                            # if the if_condition is true then the predicate must not be white:
                            cur_constraint_gate = self.generate_if_then_predicate_constraint(
                                cur_final_if_condition_output_gate, predicate, self.parsed.depth, "neg")
                            white_goal_step_output_gates.append(
                                cur_constraint_gate)
                ...
                if (self.white_single_goal_num_constraints > 1):
                    if (self.single_white_goal_disjunct_upper_limit != self.white_single_goal_num_constraints):
                        lsc.add_circuit(
                            self.gates_generator, self.single_white_goal_disjunct_boolean_variables, self.white_single_goal_num_constraints)
                        white_goal_step_output_gates.append(
                            self.gates_generator.output_gate)
        ...
        # Final goal gate:
        self.encoding.append(
            ['# And gate for white goal constraints, at index ' + str(self.parsed.depth)])
    ...
    def generate_invariant_gate(self):
        ...
        for single_conjunction_index in range(self.num_black_goal_constraints):
            ...
            if (self.num_black_goal_constraints > 1):
                self.gates_generator.if_then_gate(
                    if_disjunction_output_gate, self.gates_generator.output_gate)
            # if variables are not present due to single goal constraint, we simply add the conjunction:
            invariant_step_output_gates.append(
                self.gates_generator.output_gate)
        ...
        # Final goal gate:
        self.encoding.append(
            ['# And gate for invariant constraints, at index ' + str(self.parsed.depth)])
    ...
    def forced_propagation(self):
        ...
        for i in range(self.parsed.depth):
            self.gates_generator.complete_equality_gate(
                self.predicate_variables[i], self.predicate_variables[i+1])
            self.propagated_output_gates.append(
                self.gates_generator.output_gate)
...
# Final output gate is an nested-gate with inital, goal and transition gates:
    ...
    def generate_final_gate(self):
        ...
        for i in range(self.parsed.depth-1):
            ...
            if (reverse_index % 2 == 1):
                ...
                self.encoding.append(
                    ['# white valid constraints at reverse index: ' + str(reverse_index)])
                all_valid_constraints = []
                all_valid_constraints.append(
                    self.move_variables[reverse_index][3][0])
                all_valid_constraints.extend(
                    self.move_variables[reverse_index][4])
                self.gates_generator.and_gate(all_valid_constraints)
                valid_move_output_gate = self.gates_generator.output_gate
                ...
                if (self.makermaker_game == 1):
                    ...
                    self.gates_generator.and_gate(
                        [valid_move_output_gate, -self.move_variables[reverse_index][5][0]])
                    not_stopped_valid_gate = self.gates_generator.output_gate
                    self.gates_generator.if_then_gate(
                        not_stopped_valid_gate, cur_outgate)
                else:
                    # first implying the cur_outgate with valid move gate:
                    self.gates_generator.if_then_gate(
                        valid_move_output_gate, cur_outgate)
                ...
                if (self.makermaker_game == 1):
                    ...
                    if (self.parsed.invariant_flag == 1):
                        self.gates_generator.if_then_gate(stopped_valid_gate, [
                                                          complete_propogation_gate, self.white_goal_output_gate, self.invariant_output_gate])
                    else:
                        self.gates_generator.if_then_gate(
                            stopped_valid_gate, [complete_propogation_gate, self.white_goal_output_gate])
                    stopped_implication_gate = self.gates_generator.output_gate
                    # conjunction with this round of constraints:
                    self.gates_generator.and_gate(
                        [self.transition_step_output_gates[reverse_index], not_stopped_implication_gate, stopped_implication_gate])
                else:
                    # conjunction with this round of constraints:
                    self.gates_generator.and_gate(
                        [self.transition_step_output_gates[reverse_index], not_stopped_implication_gate])
            ...
            # for black we imply with game stop condition:
                ...
                self.gates_generator.if_then_gate(self.move_variables[reverse_index][3][0], [
                                                  self.gates_generator.output_gate, self.black_goal_output_gate, self.win_variables[reverse_index][0]])
        ...
        if (self.forced_single_output_gate == 0):
            ...
            self.gates_generator.and_gate(
                [self.initial_output_gate, cur_outgate])
        else:
            self.encoding.append(
                ['# Conjunction of Initial gate, forced gate and nested output gate: '])
            self.gates_generator.and_gate(
                [self.initial_output_gate, self.forced_single_output_gate, cur_outgate])
    ...
    def __init__(self, parsed):
        ...
        self.num_black_action_variables = int(
            math.ceil(math.log2(self.num_black_actions)))
        ...
        self.upperlimit_black_actions = int(
            math.pow(2, self.num_black_action_variables))
        # number of white actions:
        self.num_white_actions = len(self.parsed.white_action_list)
        # we need logarithmic number of variables to represent them:
        self.num_white_action_variables = int(
            math.ceil(math.log2(self.num_white_actions)))
        if (self.num_white_action_variables == 0):
            self.num_white_action_variables = 1
        self.upperlimit_white_actions = int(
        ...
        for i in range(parsed.depth):
            ...
            if (i % 2 == 0):
                ...
                temp_list.append(self.encoding_variables.get_vars(
                    self.num_black_action_variables))
            else:
        ...
        if (parsed.args.debug == 1):
            print("Number of (log) black choosing variables: ",
                  self.num_black_action_variables)
            print("Number of (log) white choosing variables: ",
                  self.num_white_action_variables)
            print("Number of (log) x index variables: ",
                  self.num_x_index_variables)
            print("Number of (log) y index variables: ",
                  self.num_y_index_variables)
            print("Move variables: ", self.move_variables)

        self.num_black_goal_constraints = len(
            self.parsed.black_goal_constraints)
        ...
        if (self.num_black_goal_constraints > 1):
            self.num_black_goal_disjunction_variables = int(
                math.ceil(math.log2(self.num_black_goal_constraints)))
            self.disjunction_goal_boolean_variables = self.encoding_variables.get_vars(
                self.num_black_goal_disjunction_variables)

            # remembering upper limit of disjunction, to handle less than constraints for choosing variables:
            self.disjunction_goal_upper_limit = int(
                math.pow(2, self.num_black_goal_disjunction_variables))
        ...
        for line in parsed.black_goal_constraints:
            for constraint in line:
                if ('?x' in constraint):
                    ...
                    temp_x_index_black_variables = self.encoding_variables.get_vars(
                        self.num_x_index_variables)
                    break
        ...
        for line in parsed.black_goal_constraints:
            for constraint in line:
                if ('?y' in constraint):
                    ...
                    temp_y_index_black_variables = self.encoding_variables.get_vars(
                        self.num_y_index_variables)
                    break
            # we only allocate the variables once:
        ...
        if (self.extra_white_quantifier_alternation == 1):
            for line in parsed.white_goal_constraints:
                for constraint in line:
                    if ('?x' in constraint):
                        # we have x indexed position:
                        temp_x_index_white_variables = self.encoding_variables.get_vars(
                            self.num_x_index_variables)
                        break
                # we only allocate the variables once:
        ...
        if (self.extra_white_quantifier_alternation == 1):
            for line in parsed.white_goal_constraints:
                for constraint in line:
                    if ('?y' in constraint):
                        # we have y indexed position:
                        temp_y_index_white_variables = self.encoding_variables.get_vars(
                            self.num_y_index_variables)
                        break
                # we only allocate the variables once:
        ...
        # for that we use boolean variables:
        # we do not need boolean variables if there is only one goal conjunction though:
        self.num_white_goal_constraints = len(
            self.parsed.white_goal_constraints)
        if (self.num_white_goal_constraints > 1):
            self.num_white_goal_conjunction_forall_variables = int(
                math.ceil(math.log2(self.num_white_goal_constraints)))
            self.forall_conjunction_white_goal_boolean_variables = self.encoding_variables.get_vars(
                self.num_white_goal_conjunction_forall_variables)
        ...
        if (self.white_single_goal_num_constraints > 1):
            self.num_white_goal_single_disjunct_boolean_variables = int(
                math.ceil(math.log2(self.white_single_goal_num_constraints)))
            self.single_white_goal_disjunct_boolean_variables = self.encoding_variables.get_vars(
                self.num_white_goal_single_disjunct_boolean_variables)

            # since existential, we need lessthan constraint in the white goal so we need the upper bound:
            self.single_white_goal_disjunct_upper_limit = int(
                math.pow(2, self.num_white_goal_single_disjunct_boolean_variables))

        if (parsed.args.debug == 1):
            print("black goal variables: ", self.black_goal_index_variables)
            print("extra quantifier alternative for white goal: ",
                  self.extra_white_quantifier_alternation)
            print("number of white single goal constraints: ",
                  self.white_single_goal_num_constraints)
            print("white goal variables: ", self.white_goal_index_variables)
            if (self.num_white_goal_constraints > 1):
                print("forall conjunction white goal boolean variables: ",
                      self.forall_conjunction_white_goal_boolean_variables)

        # Allocating forall position variables:
        self.forall_position_variables = []
        self.forall_position_variables.append(
            self.encoding_variables.get_vars(self.num_x_index_variables))
        self.forall_position_variables.append(
            self.encoding_variables.get_vars(self.num_y_index_variables))

        if (parsed.args.debug == 1):
            print("Forall position variables: ",
                  self.forall_position_variables)

        # allocating counter variables:
        if (self.parsed.counter_flag == 1):
            self.num_counter_vars = int(
                math.ceil(math.log2(self.parsed.counter_bound)))
            # if number of counter vars is 1, then the counter value is upto 2:
            if (self.num_counter_vars == 1):
                self.counter_upper_bound = 2
            else:
                self.counter_upper_bound = int(
                    math.pow(self.num_counter_vars, 2))
            ...
            for i in range(parsed.depth+1):
                self.counter_variables.append(
                    self.encoding_variables.get_vars(self.num_counter_vars))
        ...
        for i in range(parsed.depth+1):
            self.predicate_variables.append(
                self.encoding_variables.get_vars(2))
        ...
        if (self.parsed.args.sort_internal_gates == 0):
            ...
            self.gates_generator = uggen(
                self.encoding_variables, self.encoding)
