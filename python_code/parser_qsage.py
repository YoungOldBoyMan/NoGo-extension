...
def compute_index_bounds(constraints):
    ...
    for constraint in constraints:
        ...
        if ('+' in constraint or '-' in constraint):
            ...
            constraint = constraint.strip("NOT").strip(
                "black").strip("white").strip("open")
            constraint = constraint.strip("(").strip(")")
...
def combine(args):
    ...
    if ('#counterbound' in p_parsed_dict):
        ...
        f_combined_file.write(
            str(int(p_parsed_dict["#counterbound"][0][0])) + "\n")
    ...
    def __init__(self, args):
        ...
        if (self.args.game_type == "hex"):
            ...
            for neighbour_list in self.parsed_dict['#neighbours']:
                ...
                cur_position = self.rearranged_positions.index(
                    neighbour_list.pop(0))
                temp_list = []
                for neighbour in neighbour_list:
                    if (neighbour != 'na'):
                        cur_neighbour = self.rearranged_positions.index(
                            neighbour)
            ...
            if (args.tight_neighbours_with_distances == 1 and args.e == 'cp' and args.tight_neighbour_pruning == 1):
                ...
                self.tight_neighbour_pairs_list = sb.distance_tight_neighbours(
                    self)
            ...
            if (args.e == 'cp' and args.tight_neighbour_pruning == 1):
                ...
                for pair_list in self.parsed_dict['#unreachablepairs']:
                    ...
                    self.unreachable_start_end_pairs.append([self.rearranged_positions.index(
                        pair_list[0]), self.rearranged_positions.index(pair_list[1])])

            # only for explicit goals, we generate the winning configurations:
            if (args.e == 'ew' or args.e == 'eg'):
                self.black_win_configurations, self.max_win_config_length = sb.all_short_simple_paths(
                    self)
            ...
            if args.debug == 1:
                ...
                print("Upper bound of allowed moves: ",
                      self.num_available_moves)
                print("Black initial positions: ",
                      self.black_initial_positions)
                print("White initial positions: ",
                      self.white_initial_positions)
            ...
            if args.debug == 1:
                ...
                print("White initial positions: ",
                      self.white_initial_positions)
                print("Black win configurations: ",
                      self.black_win_configurations)
                print("White win configurations: ",
                      self.white_win_configurations)
            ...
            if (args.renumber_positions == 1):
                ...
                self.rearranged_positions = list(
                    self.parsed_dict['#firstmoves'][0])
            ...
            for line in self.parsed_dict['#blackgoal']:
                ...
                for single_constraint in line:
                    ...
                    single_line_constraints.append(
                        (direction, int(position_one[1:]), int(position_two[1:])))
                    self.max_num_goal_positions = max(self.max_num_goal_positions, int(
                        position_one[1:]), int(position_two[1:]))
            ...
            if args.debug == 1:
                ...
                print("Max number of goal positions: ",
                      self.max_num_goal_positions)
            ...
            if (args.ignore_file_boardsize == 0):
                self.xmax, self.ymax = int(self.parsed_dict['#boardsize'][0][0]), int(
                    self.parsed_dict['#boardsize'][0][1])
            else:
                self.xmax = args.xmax
                self.ymax = args.ymax
                if (args.debug == 1):
            ...
            if ('#counterbound' in self.parsed_dict):
                self.counter_bound = int(
                    self.parsed_dict["#counterbound"][0][0])
                self.counter_flag = 1
            ...
            for line in self.parsed_dict['#blackgoal']:
                ...
                for constraint in line:
                    ...
                    if (constraint[:2] == 'le'):
                        ...
                        bound_computation = constraint.strip(
                            ")").split(",")[-1]
            ...
            if ('#invariant' in self.parsed_dict):
                ...
                assert (len(self.black_goal_constraints)
                        == len(self.invariant_constraints))
            ...
            for i in range(len(self.white_goal_constraints)):
                ...
                if (len(self.white_goal_constraints[i]) < max_white_goal_len):
                    ...
                    num_iter = max_white_goal_len - \
                        len(self.white_goal_constraints[i])
